...
- 北京时间每天 00:00 自动采集价格并写入 Supabase；美国使用官网定价，其他地区暂时使用对应 Apple App Store storefront 的当地订阅价格。
- 页面打开时直接读取数据库快照；只有用户点击“扫描全球”才会额外发起实时扫描并写回数据库。
- 展示 OpenAI 与 Anthropic 的个人订阅级别。
- 同时展示月付、年付总价、折合月价与折扣。
- 按订阅价格从低到高排序。
- 汇总每个套餐当前可比的最低价格。
- 在 Supabase 保存最新价格与价格历史，在浏览器本地保存简单历史记录和降价提醒规则。
...
- 美国价格使用 OpenAI 与 Anthropic 官网公开定价；其他地区暂时来自对应国家或地区的 Apple App Store 公开应用内购买项目。
- 相对美国价格统一以美国官网定价为基准；全球明细表中的其他地区金额统一标注为当地公开价。
...
本项目采用 [MIT License](./LICENSE) 开源，允许使用、修改和再分发，但须保留原始版权与许可声明。
